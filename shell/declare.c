...
COLUMNS=190
COMP_WORDBREAKS=$' \t\n"\'><=;|&(:'
DBUS_SESSION_BUS_ADDRESS=unix:abstract=/tmp/dbus-BOpPasf0AA,guid=d9f34f4d5e7150a02aa7ee980000003d
...
GNOME_KEYRING_CONTROL=/tmp/keyring-9DICNG
GNOME_KEYRING_PID=1428
GROUPS=()
GTK_IM_MODULE=ibus
GTK_MODULES=canberra-gtk-module
HISTCONTROL=ignoredups:ignorespace
HISTFILE=/home/wuxiang/.bash_history
HISTFILESIZE=2000
HISTSIZE=1000
HOME=/home/wuxiang
HOSTNAME=ubuntu
...
OPTERR=1
OPTIND=1
ORBIT_SOCKETDIR=/tmp/orbit-wuxiang
OSTYPE=linux-gnu
...
PS2='> '
PS4='+ '
PWD=/home/wuxiang/language/shell
QT_IM_MODULE=xim
SESSION_MANAGER=local/ubuntu:@/tmp/.ICE-unix/1447,unix/ubuntu:/tmp/.ICE-unix/1447
SHELL=/bin/bash
SHELLOPTS=braceexpand:emacs:hashall:histexpand:history:interactive-comments:monitor
SHLVL=1
SSH_AGENT_PID=1478
SSH_AUTH_SOCK=/tmp/keyring-9DICNG/ssh
TERM=xterm
UID=1000
USER=wuxiang
USERNAME=wuxiang
WINDOWID=65013308
WINDOWPATH=7
XAUTHORITY=/var/run/gdm/auth-for-wuxiang-TujYHd/database
XDG_CONFIG_DIRS=/etc/xdg/xdg-gnome:/etc/xdg
XDG_DATA_DIRS=/usr/share/gnome:/usr/local/share/:/usr/share/
XDG_SESSION_COOKIE=77f157cc7785156d2c2c896800000009-1329961673.565236-826675267
XMODIFIERS=@im=ibus
_=--color=auto
__git_all_commands=
...
{ 
    ...
    if [[ -n $exclude ]]; then
        ...
        do
            ...
            ref="$2[$j]";
            eval $2[$j]=\${!ref}\${COMP_WORDS[i]};
            [ $i = $COMP_CWORD ] && [[ -n ${COMP_WORDS[i]} ]] && eval $3=$j;
        done;
    else
        eval $2=\( \"\${COMP_WORDS[@]}\" \);
    fi
...
' xspec;
    ...
    if [[ "$1" != -d ]]; then
        ...
        if [ ${#toks[@]} -ne 0 ]; then
            compopt &>/dev/null && compopt -o filenames || [[ ( -n ${COMP_WORDS[0]} && "$(complete -p ${COMP_WORDS[0]})" == *"-o filenames"* ) ]] || { 
                for ((i=0; i < ${#toks[@]}; i++))
                do
                    if [[ ${cur:0:1} != "'" ]]; then
...
{ 
    local cmd;
    cmd=$1;
    $cmd --help 2>&1 | command grep -- "^[[:space:]]*-" | tr "," " " | awk '{print $1; if ($2 ~ /-.*/) { print $2 } }' | sed -e "s:=.*::g"
}
_passwd () 
{ 
    local cur prev;
...
{ 
    ...
    COMPREPLY=($( command screen -ls | sed -ne         's|^[''	'']\{1,\}\('"$cur"'[0-9]\{1,\}\.[^''	'']\{1,\}\)'"$pattern"'$|\1|p' ))
}
_sdptool () 
{ 
    local cur prev split=false;
...
{ 
    ...
    if [[ "$COMP_LINE" == *$ext' ' ]]; then
        ...
        COMPREPLY=($( compgen -W "$( printf '%s ' $( tar $untar $tar             2>/dev/null ) )" -- "$cur" ));
        return 0;
    fi;
    _filedir "$ext";
    return 0
